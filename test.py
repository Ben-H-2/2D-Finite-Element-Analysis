...
def test_triangle_truss_system():
    ...
    # Check reaction forces balance with the applied load
    ...
    # Check the two diagonals carry identical stress
...
def test_triangle_element_system():
    ...
    # Check the solve recovers the applied load at the free node
    ...
    # Check the stiffness matrix is symmetric 
...
def test_multi_triangle_mesh():
    ...
    assert K.shape == (8, 8) #check matrix dimensions are correct
    assert np.allclose(K, K.T) # Check the stiffness matrix is symmetric 
